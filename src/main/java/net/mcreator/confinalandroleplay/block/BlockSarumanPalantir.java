	...
	public static class BlockCustom extends Block {
		...
		public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
			switch ((EnumFacing) state.getValue(BlockHorizontal.FACING)) {
				...
				case SOUTH :
				default :
					return new AxisAlignedBB(0.85D, 0D, 0.85D, 0.2D, 0.8D, 0.2D);
				case NORTH :
					return new AxisAlignedBB(0.15D, 0D, 0.15D, 0.8D, 0.8D, 0.8D);
				case WEST :
					return new AxisAlignedBB(0.15D, 0D, 0.85D, 0.8D, 0.8D, 0.2D);
				case EAST :
					return new AxisAlignedBB(0.85D, 0D, 0.15D, 0.2D, 0.8D, 0.8D);
			}
		}
